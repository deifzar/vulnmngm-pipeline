          ...
          stage ('Push artifacts') {
            agent {
              docker {
                ...
                reuseNode true
                args '-e JFROG_CLI_HOME_DIR=${WORKSPACE}/.jfrog-artifacts -e DOCKER_CONFIG=${WORKSPACE}/.docker-artifacts'
              }
            }
            steps {
              script {
          ...
          stage ('Push Docker image') {
            agent {
              ...
              docker {
                image 'releases-docker.jfrog.io/jfrog/jfrog-cli-full-v2-jf'
                reuseNode true
                args '-e JFROG_CLI_HOME_DIR=${WORKSPACE}/.jfrog-docker -e DOCKER_CONFIG=${WORKSPACE}/.docker-images -v /var/run/docker.sock:/var/run/docker.sock --user root'
              }
            }
            steps {
              script {
