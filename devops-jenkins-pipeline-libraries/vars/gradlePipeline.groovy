  ...
  pipeline {
    ...
    stages {
      ...
      stage('Build') {
        agent {
          docker {
            ...
            args '-e GRADLE_USER_HOME=${WORKSPACE}/.gradle' // Point Gradlew and Android SDK to write inside the workspace where we have permissions
          }
        }
        steps {
          script {
            buildHelper.buildJavaBinaryWithGradle(config)
          }
        }
      }
      ...
      stage('Test') {
        ...
        steps {
          script {
            testHelper.testJavaWithGradle(config)
          }
        }
        // Needs JUnit Plugin
        // post {
      ...
      stage('SAST') {
        ...
        steps {
          script {
            sastHelper.runSonarqubeForGradle(config)
          }
        }
      }
